defmodule SleekFlowTodo.Todos.Commands.RemoveTodoHandlerTest do
  ...
  describe "handle/2" do
    test "removes the todo when it exists" do
      ...
      aggregate_state = %Todo{todo_id: todo_id, name: "Initial Name", description: "Buy milk description 1", due_date: next_day}
      command = %RemoveTodo{todo_id: todo_id, removed_at: DateTime.utc_now()}

      assert {:ok, %TodoRemoved{todo_id: ^todo_id}} = RemoveTodoHandler.handle(aggregate_state, command)
    end
